...
def main():
    ...
    # Массивы для хранения координат и скоростей точки E
    Xe_history = []
    Ve_history = []
    Ae_history = []
    ...
    def update(frame):
        ...
        # Сохраняем скорость точки E
        Ve_history.append(VxE)
        ...
        # Сохраняем ускорение точки E
        Ae_history.append(AxE)
    ...
    plt.figure()
    plt.plot(Xe_history[1:], Ve_history[1:], 'b-')
    plt.xlabel('Xe')
    plt.ylabel('Ve')
    plt.title('Фазовый портрет точки E')
    plt.grid(True)
    plt.show()

    # Построение фазового портрета (Ae, Xe)
    plt.figure()
    plt.plot(Xe_history[1:], Ae_history[1:], 'b-')
    plt.xlabel('Xe')
    plt.ylabel('Ae')
    plt.title('Зависимость ускорения от Xe')
    plt.grid(True)
    plt.show()
