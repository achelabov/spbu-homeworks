...
line, = ax.plot([], [], 'r')
points = [ax.plot([], [], 'o', color=color)[0] for color in ['g', 'b', 'm']]
trails = [ax.plot([], [], '-', color=color, alpha=0.5)[0] for color in ['g', 'b', 'm']]
...
def update(frame):
    ...
    for i, point in enumerate(points):
        point.set_data([x1_frame[i]], [y1_frame[i]])
    
    for i, trail in enumerate(trails):
        x_trail, y_trail = trail.get_data()
        x_trail = np.append(x_trail, x1_frame[i])
        y_trail = np.append(y_trail, y1_frame[i])
