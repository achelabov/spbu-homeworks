...
# Добавим вторую точку и её след
point2, = ax.plot([], [], 'go', markersize=5)  # Вторая точка
trace2, = ax.plot([], [], 'g-', lw=1)  # След второй точки
...
def init():
    ...
    point2.set_data([], [])
    trace2.set_data([], [])
    return triangle, rod, point, trace, point2, trace2
...
def update(frame_idx):
    ...
    if iter < 30:
        ...
        if x_rot[0] > 0 and y_rot[0] < 0:
            vertex_x = xp[90]
            vertex_y = yp[90]
            mid_arc_x = xp[((N // 2 + 90) % 90) % 90]
            mid_arc_y = yp[((N // 2 + 90) % 90) % 90]
    if iter >= 30 and iter <= 60:
        ...
        if x_rot[0] > 0 and y_rot[0] > 0:
            vertex_x = xp[90]
            vertex_y = yp[90]
            mid_arc_x = xp[(N // 2 + 90) % 90]
            mid_arc_y = yp[(N // 2 + 90) % 90]
        if x_rot[0] > 0 and y_rot[0] < 0:
            vertex_x = xp[0]
            vertex_y = yp[0]
            mid_arc_x = xp[N // 2]
    ...
    # Сместим первую точку на 70% от центра к вершине
    ...
    # Добавляем координаты первой точки в след
    trace_x.append(x_point)
    trace_y.append(y_point)
    trace.set_data(trace_x, trace_y)

    # Сместим вторую точку на 50% от центра к середине дуги
    t2 = 0.2  # Коэффициент смещения (0.5 — середина, 1 — середина дуги)
    x_point2 = center_x * (1 - t2) + mid_arc_x * t2
    y_point2 = center_y * (1 - t2) + mid_arc_y * t2
    point2.set_data([x_point2], [y_point2])

    # Добавляем координаты второй точки в след
    trace2_x.append(x_point2)
    trace2_y.append(y_point2)
